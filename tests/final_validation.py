...
def test_environment_logic():
    ...
    # Do first two steps
    env.step(WorkflowAction(task_id=TaskID.medium, action_type="parse_requisition", payload={"req_id": "REQ-002"}))
    env.step(WorkflowAction(task_id=TaskID.medium, action_type="check_inventory", payload={"item_id": "ITM-002"}))
    ...
    assert obs_wrong.reward == 0.0 and obs_wrong.done, "Should fail financial validation"
    assert "Financial validation failed" in obs_wrong.info, "Should mention financial validation"
    ...
    env.step(WorkflowAction(task_id=TaskID.hard, action_type="parse_requisition", payload={"req_id": "REQ-003"}))
    ...
    print(f"   Premature message_supplier: Reward: {obs_penalty.reward:.2f}, Done: {obs_penalty.done}")
    print(f"   Info: {obs_penalty.info}")
    assert obs_penalty.reward == -0.5 and not obs_penalty.done, "Should get -0.5 penalty"
    assert "Penalty: message_supplier before check_inventory" in obs_penalty.info, "Should mention penalty"
    print("   ✅ PASS - Correctly applied penalty")
    ...
    for i, (action_type, payload) in enumerate(steps, 1):
        action = WorkflowAction(task_id=TaskID.hard, action_type=action_type, payload=payload)
        obs = env.step(action)
        total_reward += obs.reward
        print(f"   Step {i} ({action_type}): Reward: {obs.reward:.2f}, Done: {obs.done}")
